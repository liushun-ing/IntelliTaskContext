...
public class TargetGraphOperator {
  ...
  /**
   * 根据Task_Context中的所有种子元素(PsiField,PsiMethod,PsiClass)构建目标图
   *
   * @return 目标图
   */
  public static Graph buildTargetGraph() {
    ...
    for (int i = root.getChildCount() - 1; i >= 0; i--) {
      ...
      PsiElement psiElement = (PsiElement) userObject.getCaptureElement();
      ...
      for (int j = childAt.getChildCount() - 1; j >= 0; j--) {
        ID++;
        ...
        PsiElement psiElementChild = (PsiElement) userObject1.getCaptureElement();
        if (psiElementChild instanceof PsiField) {
          PsiField psiField = (PsiField) psiElementChild;
          if (elementIsInProject(psiField.getContainingClass())) {
            Vertex fieldVertex = new Vertex(ID, "FIELD", psiElementChild);
            vertices.add(fieldVertex);
          }
        } else if (psiElementChild instanceof PsiMethod) {
          PsiMethod psiMethod = (PsiMethod) psiElementChild;
          if (elementIsInProject(psiMethod.getContainingClass())) {
            Vertex originVertex = new Vertex(ID, "METHOD", psiElementChild);
            vertices.add(originVertex);
          }
        } else if (psiElementChild instanceof PsiClass) {
          PsiClass psiClass = (PsiClass) psiElementChild;
          if (elementIsInProject(psiClass)) {
            Vertex originVertex = new Vertex(ID, "CLASS", psiElementChild);
            vertices.add(originVertex);
          }
        } else {
          ID--;
    ...
    // 根据种子扩展目标图
    if (DataCenter.PREDICTION_STEP == 1) {
      extendOneStepGraph(vertices, edges);
    } else if (DataCenter.PREDICTION_STEP == 2) {
      extendTwoStepGraph(vertices, edges);
    } else if (DataCenter.PREDICTION_STEP == 3) {
   ...
   * @return 原来的节点数
   */
  public static int extendOneStepGraph(ArrayList<Vertex> vertices, ArrayList<Edge> edges) {
  ...
  public static int extendTwoStepGraph(ArrayList<Vertex> vertices, ArrayList<Edge> edges) {
    ...
    int start = extendOneStepGraph(vertices, edges);
    int length = vertices.size();
